...
unless ENV['MOJACO_MYSQL_USER'] && ENV['MOJACO_POSTGRESQL_USER']
...
RSpec.configure do |config|
  ...
  config.before(:each) do
    [MysqlTestClient, PostgresqlTestClient].each do |sql_client|
    ...
    # DatabaseCleaner does not seem to work with mongoid so we clean it manually
    raise Exception.new('Wrong DB while running test!! Use dataflow_test') if Mongoid::Clients.default.database.name != 'dataflow_test'
    # Drop the whole test db after each test
    Mongoid::Clients.default.database.drop

    # Delete all csv files used for tests.
    Dir["./datanodes/csv/dataflow_test.*.csv"].each { |f| File.delete(f) }
  end
